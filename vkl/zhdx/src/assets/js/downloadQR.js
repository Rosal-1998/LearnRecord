...
export default {
    ...
    axios
      ...
      .then((res) => {
        ...
        if (res.headers['content-disposition'] != undefined) {
          ...
          // 尝试解析 filename* 参数（如果存在）
          let matches = contentDisposition.match(/filename\*=([^;]+)/i)
          if (matches) {
            // RFC 5987 定义，可能需要进行进一步处理（如百分比解码）
            // 这里只处理了引号内的部分，但实际上还需要处理字符集等
            let encodedFilename = matches[1].split("'")[1] // 假设引号正确使用
            filename = decodeURIComponent(encodedFilename) // 注意：这里可能需要先按字符集解码
          } else {
            // 使用 filename= 参数
            ...
            if (matches) {
              ...
              matches = contentDisposition.match(/filename=([^;]+)/i)
              if (matches) {
                filename = decodeURIComponent(matches[1].trim()) // 可能需要URI解码
              }
            }
          }
        } else {
  ...
  chooseimg: function (val) {
    ...
    isLt =
      inputDOM.files[0].size / 1024 / 5 >= 1 &&
      inputDOM.files[0].size / 1024 / 1024 / 10 <= 1
    if (!isLt) {
  ...
  uploadimg: function (type) {
    ...
    axios
      .post('/admin/sys_user/import?t=' + random, formData, {
        headers: {
