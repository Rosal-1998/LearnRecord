		...
		<div class="m-contain">
			...
			<div class="m-table">
				...
				:cell-style="{'text-align':'center'}">
					...
					<el-table-column label="ID" prop="patrolTemplateId">
					</el-table-column>
					...
					<ul class="record-box">
						<li class="record-lst" v-for="(item,index) in recordlist">
							...
							<div class="rec-rt">
								...
								<span class="bbh"><i class="name">检查单文件:</i><i class="yl-btn" @click="handleyl(item)">下载</i><i class="file" :title="item.patrolTemplateCheckFileNames">{{item.patrolTemplateCheckFileNames}}</i> </span>
		...
		data(){
			return{
				...
				addFormRules:{
					...
					patrolTemplateType:[{required:true, message:'请选择巡查类型',trigger:["blur",'change']}]
				},
				filelist:[],
				syqkdialog:false,
				patrolTemplateId:'',
