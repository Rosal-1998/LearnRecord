<template>
	<div class="g-cont">
		...
		<div class="g-rtside">
			<div class="m-search" style="float: left;">
				<el-form :model="filters" :inline="true" ref="filters">
					...
					<el-form-item label="处理节点" prop="evtNodeFlowId">
						<el-select v-model="filters.evtNodeFlowId" placeholder="展示状态">
			...
			<span class="btn add-btn" style="margin-bottom: 0.08333rem" v-if="current == '2'" @click="handlefq">发起</span>
			<div class="m-table">
				...
				:cell-style="{'text-align':'center'}">
					...
					<el-table-column label="操作" prop="" width="200">
						<template slot-scope="scope">
							<div v-if="current == '0'">
								<span class="edit-title" v-if="scope.row.evtNodeFlowId == 'Activity_sjsb001'" @click="reportagain(scope.row)">处理</span>
							...
							<div v-if="current == '2'">
								<span class="edit-title" v-if="scope.row.evtNodeResult == 'unreported' && scope.row.evtNodeFlowId =='Activity_sjsb001'" @click="reportagain(scope.row)">重新上报&nbsp;&nbsp;</span>
								<span class="del-title" v-if="scope.row.evtNodeResult == 'unreported' && scope.row.evtNodeFlowId =='Activity_sjsb001'" @click="handledel(scope.row)">删除&nbsp;&nbsp;</span>
								<span class="edit-title" v-if="scope.row.evtNodeResult =='unhandled' && scope.row.evtNodeFlowId == 'Activity_sjsl002'" @click="handlech(scope.row)">撤回&nbsp;&nbsp;</span>
								<span class="detail-title" v-if="scope.row.evtNodeFlowId !='Activity_sjsb001' || scope.row.evtNodeFlowId != 'Activity_sjsl002'"  @click="handldetail(scope.row)">详情</span>
							</div>
							<div v-if="current == '3'">
								<span class="detail-title" @click="handldetail(scope.row)">详情</span>
							</div>
						...
						<el-form-item label="附件" prop="">
							<div class="file-lst">
								...
								<ul class="picul" v-if="piclist.length>0">
									<li class="pic-lst" v-for="(item,index) in piclist" @click="delfile(index,'0')">
						...
						<el-form-item label="附件1" prop="" class="noname">
							<div class="file-lst">
								...
								<ul class="picul" v-if="audiolist.length>0">
									<li class="pic-lst aud-lst" v-for="(item,index) in audiolist" @click="delfile(index,'1')">
						...
						<el-form-item label="附件2" prop="" class="noname">
							<div class="file-lst">
								...
								<ul class="picul" v-if="videolist.length>0">
									<li class="pic-lst" v-for="(item,index) in videolist" @click="delfile(index,'2')">
						...
						<li class="record-lst" v-for="(item,index) in recordlist">
							...
							<div class="rec-rt">
								...
								<div class="com-ul" v-if="item.commentList && item.commentList.length>0">
									<div v-for="(items,index) in item.commentList" class="com-lst">{{items.fullMessage}}</div>
								</div>
							</div>
						</li>
					</ul>
		...
		<el-dialog :visible.sync="dtdialog" v-if="dtdialog" class="g-dialog" title="事件上报">
			...
			<div class="hqzdt">
				<baidu-map style="height: 100%;" :center="mapCenter" :zoom="mapZoom" :scroll-wheel-zoom="true" ak="baidu-ak"
					@ready="handlerBMap" @click="getClickInfo" />
			</div>
		...
		<el-dialog :visible.sync="xqdialog" v-if="xqdialog" class="g-dialog" title="事件处理详情">
			...
			<!-- 受理 -->
			<div class="dialog-foot four-btn" v-if="current =='0'&& evtNodeFlowId == 'Activity_sjsl002'">
...
<script>
	...
	import {EventSidebar, EventList,EventType ,EventNode ,EventDetil, DictType, GetReportForm, EvtFlowStart
		,EvtFlowRevo,EvtFlowBackReport,EvtReportAgain,EvtFlowDelete
	} from '../../api/api.js'
	import { EvtFlowAllot, EvtFlowDisSign ,EvtFlowDisRefuseSign, EvtFlowSubCon, EvtFlowSelfDisComp,EvtFlowSuspend ,EvtFlowConfbCon
,EvtFlowBackDis, SelectDeptUserTree} from '../../api/api.js' //受理并分派
	import { EvtFlowCheck,EvtFlowCheckSign,EvtFlowCheckRefuseSign,EvtFlowCheckComplete,EventRecall,EvtFlowAgreeSuspend
,EvtFlowRefuseSuspend,EvtFlowCancSuspend,EvtFlowTransfer } from '../../api/api.js' // 发起核查
		...
		methods:{
			...
			getlist:function(val){
				if(this.filters.time && this.filters.time.length>0){
					this.filters.evtNodeReceiptBeginTime = moment(this.filters.time[0]).format('yyyy-MM-DD')
					this.filters.evtNodeReceiptEndTime = moment(this.filters.time[1]).format('yyyy-MM-DD')
				}else{
