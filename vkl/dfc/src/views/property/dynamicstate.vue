				...
				:cell-style="{'text-align':'center'}">
					...
					<el-table-column label="序号" width="80">
						<template slot-scope="scope">
							<el-popover placement="top" width="160" v-model="scope.row.visible" @show="ruleForm.serialNum = null">
								<el-form :model="ruleForm" :rules="rules" ref="ruleForm" class="demo-ruleForm">
									<el-form-item prop="serialNum">
										<el-input type="number" placeholder="请输入序号" v-model="ruleForm.serialNum"/>
									</el-form-item>
									<el-form-item>
										<el-button @click="scope.row.visible = false">取消</el-button>
										<el-button type="primary" @click="confirmUpdateSerial('ruleForm', scope.$index)">确定</el-button>
									</el-form-item>
								</el-form>
								<div slot="reference" style="cursor: pointer">{{scope.row.sort}}</div>
							</el-popover>
						</template>
					</el-table-column>
					...
					<el-table-column label="发布时间" prop="publishTime" show-overflow-tooltip></el-table-column>
...
<script>
	import {NewsInforList, AddNewsInfor, UpdateNewsSerial, UpdateNewsInfor ,DelNewsInfor, NewsInforfindById, ChangestatusNewsInfor, DictType} from '../../api/api.js'
	...
	export default {
		...
		data(){
          ...
          serialNum: []
				...
				addFormRules:{
					newsTitle:[{required:true, message:'请输入动态标题',trigger:'blur'}],
					describes:[{required:true, message:'请输入动态内容描述',trigger:'blur'}],
					displayStatus:[{required:true, message:'请选择展示状态',trigger:'change'}],
					newsType:[{required:true, message:'请选择动态类型',trigger:'change'}],
		...
		mounted(){
			...
			this.rules.serialNum = [{ validator: checkNumber, trigger: 'blur' }]
			this.addFormRules.sort[1] = { validator: checkNumber, trigger: 'blur' }
		...
		methods:{
			// 序号的修改
			updateSerial(index) {
				this.datalist[index].visible = true
			},
			confirmUpdateSerial(formName, index) {
				this.$refs[formName].validate((valid) => {
          if (valid) {
						const param = {
							newsId: this.datalist[index].newsId,
							sort: this.ruleForm.serialNum
						}
						UpdateNewsSerial(param).then(res => {
							if (res.code == 200) {
								this.$message({
									message: '修改成功',
									type: 'success'
								})
								this.datalist[index].visible = false
								this.getlist('')
							}
						})
          } else {
