<template>
  <div class="g-cont">
    ...
    <div class="g-rtside">
        ...
        <el-form :model="filters" :inline="true" ref="filters">
          <el-form-item label="事件类型" prop="evtTypeId">
            <el-select v-model="filters.evtTypeId" placeholder="事件类型">
              <el-option
                v-for="(item, index) in eventypelist"
          ...
          <el-form-item label="处理节点" prop="evtNodeFlowId">
            <el-select v-model="filters.evtNodeFlowId" placeholder="处理节点">
              <el-option
                ...
                :value="item.evtNodeFlowId"
                :label="item.evtNodeName"
                :key="index"
              ></el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="发起人" class="ipt150" prop="evtInitiatorName">
            <el-input
              v-model="filters.evtInitiatorName"
              placeholder="发起人"
            ></el-input>
          </el-form-item>
      ...
      <div class="m-table">
        ...
        >
          <span class="btn search-btn" style="margin-right: 20px;" @click="handleOnce()"
            >批量处理</span
          >
  ...
  <p class="del-tip">是否删除事件？</p>
    ...
    <span slot="reference" class="btn del-btn" style="float: left" 
            >批量删除</span
          >
  </el-popover></template>
        </div>
        ...
        >
          ...
          <el-table-column label="事件类型" prop="evtTypeName" width="120">
          </el-table-column>
          <el-table-column label="处理节点" prop="evtNodeName" width="120">
          </el-table-column>
          <el-table-column label="处理结果" prop="" width="120">
            <template slot-scope="scope">
              <span
                class="f-orange"
                v-if="scope.row.evtNodeResultName == '待处理'"
          ...
          <el-table-column label="操作" prop="" width="200">
            <template slot-scope="scope">
              <div v-if="current == '0'">
                <span
                  class="edit-title"
                  v-if="scope.row.evtNodeFlowId == 'Activity_sjsb001'"
                  @click="reportagain(scope.row)"
                  >处理</span
                >
                <span class="edit-title" v-else="" @click="handldeal(scope.row)"
                  >处理</span
                >
              </div>
              <div v-if="current == '1'">
    ...
    >
        ...
        <div class="lt">
          ...
          >
            ...
            <el-form-item label="事件描述" prop="evtDescription">
              <el-input
                type="textarea"
                v-model="addForm.evtDescription"
                placeholder="请输入事件描述"
              ></el-input>
            </el-form-item>
            ...
            <el-form-item label="附件" prop="" class="noname">
              <div class="file-lst">
                <div class="filebox">
                  <input
                    type="file"
          ...
          <ul class="record-box">
            <li class="record-lst" v-for="(item, index) in recordlist">
              ...
              <div class="rec-rt">
                ...
                >
                  <div
                    v-for="(items, index) in item.commentList"
                    class="com-lst"
                  >
                    {{ items.fullMessage }}
                  </div>
      ...
      <div class="dialog-foot" v-if="current == '0'">
        ...
        <span class="btn-save" @click="SubmitEvent('0')" v-if="nodetype == '1'"
          >重新上报</span
        >
      </div>
      <div class="dialog-foot" v-if="current == '2'">
        <span class="btn-cancel" @click="handlecancel('0')">取消</span>
        <span class="btn-save" @click="handlesb" v-if="nodetype == '0'"
          >上报</span
        >
        <span class="btn-save" @click="SubmitEvent('0')" v-if="nodetype == '1'"
          >重新上报</span
    ...
    >
          ...
          <ul class="record-box">
            <li class="record-lst" v-for="(item, index) in recordlist">
              ...
              <div class="rec-rt">
                ...
                >
                  <div
                    v-for="(items, index) in item.attachmentList"
                    class="com-lst"
                  >
                    附件：{{ items.name }}
                  </div>
      ...
      <!-- 受理 -->
      <div
        class="dialog-foot five-btn"
        v-if="current == '0' && evtNodeFlowId == 'Activity_sjsl002'"
      >
        <span
          class="btn-cx"
          style="margin-left: 0; background: #00a0e9; border: 1px solid #00a0e9"
          @click="yjclbtn"
          >一键处理</span
        >
      ...
      <div
        class="dialog-foot"
        v-if="current == '0' && evtNodeFlowId == 'Activity_sjczqs005'"
      >
        <span class="btn-cx" style="margin-left: 0" @click="handleEvent('5')"
          >拒签</span
        >
      ...
      <div
        class="dialog-foot"
        v-if="current == '0' && evtNodeFlowId == 'Activity_sjhcqs003'"
      >
        <span class="btn-cx" style="margin-left: 0" @click="handleEvent('7')"
          >拒签</span
        >
      ...
      <div
        class="dialog-foot"
        v-if="current == '0' && evtNodeFlowId == 'Activity_sjhc004'"
      >
        <span class="btn-save" @click="handleEvent('13')">核查完成</span>
      </div>
      <!-- 提交办结 自处置完成 挂起 转派-->
      <div
        class="dialog-foot four-btn"
        v-if="current == '0' && evtNodeFlowId == 'Activity_sjcz006'"
      >
        <span class="btn-save" style="margin-left: 0" @click="handleEvent('8')"
          >提交办结</span
        >
      ...
      <div
        class="dialog-foot"
        v-if="current == '0' && evtNodeFlowId == 'Activity_sjgqsp007'"
      >
        <span class="btn-cx" style="margin-left: 0" @click="handleEvent('15')"
          >拒绝</span
        >
      ...
      <div
        class="dialog-foot"
        v-if="current == '0' && evtNodeFlowId == 'Activity_sjgq009'"
      >
      ...
      <div
        class="dialog-foot"
        v-if="current == '0' && evtNodeFlowId == 'Activity_sjbj008'"
      >
        <span class="btn-cx" style="margin-left: 0" @click="handleEvent('12')"
          >退回处置</span
        >
    ...
    >
      ...
      <div class="dialog-foot">
        ...
        <span class="btn-save" v-if="yjjdtype == '0'" @click="slfpsubmit"
          >受理并分派</span
        >
        <span class="btn-save" v-if="yjjdtype == '1'" @click="handledete('0')"
          >事件撤销</span
        >
        <span class="btn-save" v-if="yjjdtype == '2'" @click="hcsubmit"
          >发起核查</span
        >
        <span class="btn-save" v-if="yjjdtype == '3'" @click="handledete('1')"
          >回退</span
        >
        <span class="btn-save" v-if="yjjdtype == '4'" @click="signSubmit('0')"
          >签收</span
        >
        <span class="btn-save" v-if="yjjdtype == '5'" @click="signSubmit('1')"
          >拒签</span
        >
        <span
          class="btn-save"
          v-if="yjjdtype == '6'"
          @click="checksignSubmit('0')"
          >签收</span
        >
        <span
          class="btn-save"
          v-if="yjjdtype == '7'"
          @click="checksignSubmit('1')"
          >拒签</span
        >
        <span class="btn-save" v-if="yjjdtype == '8'" @click="eventsubmit('0')"
          >提交办结</span
        >
        <span class="btn-save" v-if="yjjdtype == '9'" @click="eventsubmit('1')"
          >自动完成处置</span
        >
        <span class="btn-save" v-if="yjjdtype == '10'" @click="eventsubmit('2')"
          >挂起</span
        >
        <span
          class="btn-save"
          v-if="yjjdtype == '11'"
          @click="eventcomplet('0')"
          >确认办结</span
        >
        <span
          class="btn-save"
          v-if="yjjdtype == '12'"
          @click="eventcomplet('1')"
          >退回处置</span
        >
        <span class="btn-save" v-if="yjjdtype == '13'" @click="checksubmit"
          >核查完成</span
        >
        <span class="btn-save" v-if="yjjdtype == '14'" @click="eventgq('0')"
          >同意挂起</span
        >
        <span class="btn-save" v-if="yjjdtype == '15'" @click="eventgq('1')"
          >拒绝挂起</span
        >
        <span class="btn-save" v-if="yjjdtype == '16'" @click="eventgq('2')"
          >取消挂起</span
        >
        <span class="btn-save" v-if="yjjdtype == '17'" @click="zpsubmit"
          >转派</span
        >
      </div>
    </el-dialog>
...
export default {
  ...
  methods: {
    ...
    // 事件类型
    getsjlx: function () {
      EventType({ evtParentType: 'securityEvent' }).then((res) => {
