<template>
  <div class="g-cont">
    <div class="m-contain">
      <span class="btn add-btn" @click="handleAdd">添加居民</span>
      ...
      <div class="m-search" style="margin-top: 0.083rem; float: left">
        <el-form :model="filters" :inline="true" ref="filters">
          <el-form-item label="是否来沪人员" prop="outsider">
            <el-select v-model="filters.outsider">
              <el-option
                :value="item.value"
                :label="item.label"
                :key="index"
          ...
          <el-form-item label="是否办理居住证" prop="residencePermit">
            <el-select v-model="filters.residencePermit">
              <el-option
                :value="item.value"
                :label="item.label"
                :key="index"
          ...
          <el-form-item label="户籍" prop="registered">
            <el-input
          ...
          <el-form-item label="标签" prop="tags">
            <el-input v-model="filters.tags" placeholder="标签"></el-input>
          </el-form-item>
          <el-form-item label="联系方式" prop="phone">
            <el-input v-model="filters.phone" placeholder="联系方式"></el-input>
          </el-form-item>
          <el-form-item label="年龄" prop="year" class="year">
            <el-input v-model="filters.yearmax" placeholder="最大"></el-input
            >至<el-input
      ...
      <div class="m-table">
        ...
        >
          ...
          <el-table-column label="是否户主" prop="">
            <template slot-scope="scope">
              <span class="f-blue" v-if="scope.row.houseHolder == '0'">是</span>
          ...
          <el-table-column label="户主" prop="residentName"> </el-table-column>
          <el-table-column label="是否来沪人员" prop="" width="130">
            <template slot-scope="scope">
          ...
          <el-table-column label="是否办理居住证" prop="" width="130">
            <template slot-scope="scope">
              <span class="f-blue" v-if="scope.row.residencePermit == 'yes'"
                >是</span
              >
              <span class="f-gray" v-else-if="scope.row.residencePermit == 'no'"
                >否</span
              >
              <span v-else="">/</span>
            </template>
          ...
          <el-table-column label="操作" prop="" width="150">
            <template slot-scope="scope">
              ...
              >&nbsp;&nbsp;
              <el-popover
                :ref="scope.row.residentId"
                placement="top"
                width="160"
                popper-class="popover"
              >
                <p class="del-tip">是否删除该居民信息？</p>
                <div class="pop-btnbox">
                  <span
                    class="cancel-btn"
                    @click="visibleDelCancel(scope.row.residentId)"
                    >取消</span
                  >
                  <span
                    class="ok-btn"
                    @click="visibleDelSubmit(scope.row.residentId)"
                    >确定</span
                  >
                </div>
    ...
    >
      ...
      <div class="xq-content">
        <div class="xq-lst" style="width: 100%">
          ...
          <img
            v-if="detailform.residentAvatarFile"
            :src="detailform.residentAvatarFile"
            class="jmzp"
          />
        </div>
        <div class="xq-lst">
        ...
        <div class="xq-lst">
          ...
          <span class="lab" v-else-if="detailform.sex == '2'">未知</span>
        ...
        <div class="xq-lst">
          ...
          <span class="lab">{{ detailform.room }}</span>
        </div>
        <div class="xq-lst">
          <span class="name">是否户主</span>
          <span class="lab" v-if="detailform.houseHolder == '0'">是</span>
          <span class="lab" v-else-if="detailform.houseHolder == '1'">否</span>
          <span class="lab" v-else="">/</span>
        </div>
        <div class="xq-lst">
          <span class="name">户主姓名</span>
          <span class="lab">{{ detailform.residentName }}</span>
        </div>
        <div class="xq-lst">
          <span class="name">与户主关系</span>
          <span class="lab">{{ detailform.relationship }}</span>
        </div>
        <div class="xq-lst">
          <span class="name">民族</span>
          <span class="lab">{{ detailform.nation }}</span>
        </div>
        <div class="xq-lst">
          <span class="name">政治面貌</span>
          <span class="lab">{{ detailform.politicsFace }}</span>
        </div>
        <div class="xq-lst">
          <span class="name">婚姻状况</span>
          <span class="lab">{{ detailform.marriage }}</span>
        </div>
        <div class="xq-lst">
        ...
        <div class="xq-lst">
          ...
          <span class="lab">{{ detailform.health }}</span>
        </div>
        <div class="xq-lst">
          <span class="name">户籍地址</span>
          <span class="lab">{{ detailform.registered }}</span>
        </div>
        <div class="xq-lst">
          <span class="name">居住地派出所</span>
          <span class="lab">{{ detailform.locationPcs }}</span>
        </div>
        <div class="xq-lst">
          <span class="name">居住地址</span>
          <span class="lab">{{ detailform.location }}</span>
        </div>
        <div class="xq-lst">
          <span class="name">户号</span>
          <span class="lab">{{ detailform.locationHh }}</span>
        </div>
        <div class="xq-lst">
        ...
        <div class="xq-lst">
          ...
          <span class="lab">{{ detailform.workUnit }}</span>
        </div>
        <div class="xq-lst">
          <span class="name">获得荣誉</span>
          <span class="lab">{{ detailform.honor }}</span>
        </div>
        <div class="xq-lst" style="width: 100%">
          <span class="name">是否来沪人员</span>
          <span class="lab" v-if="detailform.outsider == '0'">是</span>
          <span class="lab" v-else-if="detailform.outsider == '1'">否</span>
          <span class="lab" v-else="">/</span>
        </div>
        <div class="xq-lst" style="width: 100%">
          <span class="name">是否办理居住证</span>

          <span class="lab" v-if="detailform.residencePermit == 'yes'">是</span>
          <span class="lab" v-else-if="detailform.residencePermit == 'no'"
            >否</span
          >
          <span class="lab" v-else="">/</span>
        </div>
    ...
    >
      ...
      <div class="xq-content car-detail">
        <div class="xq-lst">
          ...
          <span class="lab">{{ cardetailform.vehicleOwner }}</span>
        </div>
        <div class="xq-lst">
          <span class="name">车辆所属房屋</span>
          <span class="lab">{{ cardetailform.vehicleHouse }}</span>
        </div>
        <div class="xq-lst">
          <span class="name">车牌号</span>
          <span class="lab">{{ cardetailform.vehicleNo }}</span>
        </div>
        <div class="xq-lst">
          <span class="name">车辆类型</span>
          <span class="lab">{{ cardetailform.typeName }}</span>
        </div>
        <div class="xq-lst">
          <span class="name">照片</span>
          <img :src="cardetailform.vehicleImgFile" class="carimg" />
        </div>
      </div>
    </el-dialog>
  </div>
